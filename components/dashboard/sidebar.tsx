...
export function Sidebar() {
  ...
  return (
    <>
      {/* Mobile Top Bar */}
      <div className="lg:hidden flex items-center justify-between bg-white border-b border-slate-200 p-4 sticky top-0 z-20">
        <div className="flex items-center gap-2 text-slate-900 font-bold">
          <CalendarCheck className="h-6 w-6 text-blue-700" />
          BookZy
        </div>
        <button onClick={toggle} className="text-slate-600 focus:outline-none">
      ...
      {/* Sidebar Content */}
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-slate-200 flex flex-col transition-transform duration-200 ease-in-out lg:translate-x-0 lg:static lg:h-screen ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="p-6 hidden lg:flex items-center gap-2 text-slate-900 font-bold text-xl">
