...
export function Dialog({ open, onOpenChange, children }: DialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      />
      <div className="relative z-50 w-full max-w-lg rounded-xl bg-white p-6 shadow-xl border border-slate-200 m-4 animate-in fade-in zoom-in-95 duration-200">
        {children}
      </div>
    </div>
  );
...
export function DialogTitle({ className, children }: React.HTMLAttributes<HTMLHeadingElement>) {
  return (
    <h2 className={cn("text-xl font-semibold leading-none tracking-tight text-slate-950", className)}>
      {children}
    </h2>
  );
}
